...
import { getAddressById, updateAddress, deleteAddress } from "@/config/api";
...
const AddressDetail = () => {
    ...
    const [address, setAddress] = useState<IAddressUser>();
    ...
    const fetchAddress = async () => {
        if (id) {
            const res = await getAddressById(id as string);
            if (res.data) {
                setAddress(res.data);
                setIsDefault(res.data.defaultAddress);
    ...
    return (
        <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : "height"} style={styles.container}>
            <HeaderList title="Chi tiết địa chỉ" backPress={() => router.back()} showAdd={false} />
            <Formik
                enableReinitialize
                initialValues={{
                ...
                onSubmit={async (values) => {
                    ...
                    const res = await updateAddress(addressUser);
                    if (res.data) {
                        Toast.show({
            ...
            >
                {({ handleChange, handleBlur, handleSubmit, values, errors, touched, setFieldValue }) => (
                    <ScrollView style={{ flex: 1, backgroundColor: COLORS.BACKGROUND, paddingBottom: 40 }}>
                        ...
                        <ShareButton title="Xóa địa chỉ" onPress={() => setShowConfirmDialog(true)}
                            btnStyle={{
                                marginHorizontal: 25,
