...
import { callGetOrders, callUpdateOrder, callDeleteOrder } from "@/config/api";
...
const OrdersScreen = () => {
    ...
    const createFilter = (filterName: string, filterStatus: string) => {
        ...
        if (filterName.length > 0) {
            filters.push(`user.name~'*${filterName}*'`);
        }
    ...
    }, [filterStatus, filterName, sort, selectedIndex, isRefreshing, startDate, endDate, minPrice, maxPrice]);
    ...
    const fetchOrders = async (filter?: string) => {
        try {
            ...
            Toast.show({
                text1: "Lỗi khi tải danh sách đơn hàng",
                type: "error"
            });
        }
    };
    ...
    const handleViewOrder = (item: IOrderRes) => {
        // const path = "/management/orders/OrderDetail";
        router.push({
        ...
        <KeyboardAvoidingView behavior="padding" style={styles.container}>
            <View style={styles.container}>
                ...
                <FlatList
                    ...
                    ListEmptyComponent={<EmptyState title="Không có đơn hàng" description="Chưa có đơn hàng nào được tạo" />}
                ...
                {/* Advanced Filter Modal - Fixed with KeyboardAvoidingView */}
                <Modal
...
const styles = StyleSheet.create({
    ...
    // Modal styles - Fixed for keyboard handling
    modalKeyboardView: {
        flex: 1,
    },
    modalOverlay: {
